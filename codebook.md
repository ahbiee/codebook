...
2.3 Prefix Sum(前綴和) 和 Difference(差分) . . . . . . . . . . .
2.4 Monotonic Stack 單調棧 . . . . . . . . . . . . . . . . . . .
2.5 Monotonic Queue 單調隊列 . . . . . . . . . . . . . . . . . .
...
9
6.1 點與向量基礎 (Point & Vector) . . . . . . . . . . . . . . . . 10
6.2 判斷線段、矩形相交 (Segment Intersection) . . . . . . . . . 10
6.3 點與線段關係 (Point & Line) . . . . . . . . . . . . . . . . . 10
6.4 任意多邊形面積 . . . . . . . . . . . . . . . . . . . . . . . . 11
6.5 凸包 Convex Hull
. . . . . . . . . . . . . . . . . . . . . . . 11
...
12
8.1 01 Knapsack 背包 . . . . . . . . . . . . . . . . . . . . . . . 12
8.2 LCS 最長共同子序列 (Sequence) . . . . . . . . . . . . . . . 13
8.3 LIS / LDS 最長遞增/遞減子序列 . . . . . . . . . . . . . . . 13
8.4 LCS 轉 LIS . . . . . . . . . . . . . . . . . . . . . . . . . . 14
8.5 區間 DP 概念 (最長迴文子序列)
. . . . . . . . . . . . . . . 14
8.6 跳格子 . . . . . . . . . . . . . . . . . . . . . . . . . . . . . 15
8.7 分錢幣問題 . . . . . . . . . . . . . . . . . . . . . . . . . . . 15
8.8 Kadane’s Algorithm (最大子陣列和) . . . . . . . . . . . . . 15
...
9.8 任務調度問題 . . . . . . . . . . . . . . . . . . . . . . . . . . 16
9.9 多機調度問題 . . . . . . . . . . . . . . . . . . . . . . . . . . 17
9.10 最大區間重疊數 Sweep Line . . . . . . . . . . . . . . . . . . 17
9.11 Josephus 約瑟夫問題 . . . . . . . . . . . . . . . . . . . . . 17

10 Search & Techniques

17
10.1 Binary Search . . . . . . . . . . . . . . . . . . . . . . . . . 17
10.2 Ternary Search . . . . . . . . . . . . . . . . . . . . . . . . 17
10.3 Two Pointer 雙指針 . . . . . . . . . . . . . . . . . . . . . . 18
10.4 Sliding Window 滑動窗口 . . . . . . . . . . . . . . . . . . . 18
...
用法: Prefix sum: 多次區間查詢/區間和，Difference: 大量區間更新
...
// 如果是 XOR 就把 + 換 ^ -> pre[i] = pre[i-1] ^ a[i];

return pre[r] - pre[l - 1];
// XOR(l,r) = pre[r] ^ pre[l-1];

10 // 查詢含 l,r 的區間 query [l, r]
11 ll query(int l, int r) {
12
13
14 }
15
...
35 // === 1D Difference ===
36
37 // 區間加 val -> 從 l + val，到 r+1 不再加 val
38 diff[l] += val;
39 diff[r + 1] -= val;
40
41 // 還原值
...
59
60 }
61 // 此時 diff[i][j] 是 " 經過處理後的值"，如果有原始陣列 a，需再加上 a[i][j]
...
// 第一輪會跳過，因為 st 是 empty
while (!st.empty() && a[st.top()] < a[i]) {

用法: 找類似 Next Greater Element 這樣的題目
時間複雜度: O(N )
1 // === Next Greater Element ===
2 int a[105] = {}; // 存好值的 array
3 int ans[105] = {}; // 存答案的 array
4 stack<int> st; // 注意是存 index 不是 value，value 在 a[st.top()]
5
6 for (int i = 0; i < n; i++) {
7
8
...
22 只需要把 while 改成
23 while (!st.empty() && a[st.top()] > a[i]) // (因為要找更小)
24
25 // === Previous Greater ===
...
用法: 直接用來找 Sliding Window Maximum/Minimum，使用 deque
時間複雜度: O(N )
...
1 deque<int> dq; // 注意一樣是存 index
2 int k = 5; // 最大窗口大小 maximum window length
3
4 // 以尋找最長遞減序列為例 (maximum)
5 for (int i = 0; i < n; i++) {
6
7
8
...
5
6 對於 ax + by = c，若 gcd(a, b) 不能整除 c (c 不是他的倍數)，就代表沒有整數解
7 否則，設 g = gcd(a, b)，先解出基本解 x', y'
8 */
9 long long extgcd(long long a, long long b, long long &x, long long &y) {
...
19 }
20
21 // 萬能模逆元：求 a 在 mod m 之下的逆元 (無論 m 是否為質數皆可)，-1 表示不存在
22 long long inverse(long long a, long long m) {
23
24
25
26
...
用法: 快速計算
時間複雜度: O(
1 // 應用場景：題目要你算從 i ~ N 中 N / i 取下高斯後的總和分數，sum = sum(

⌋。利用商的階梯狀分布，將 O(N ) 降至 O(

⌊ N
i
N )

floor(n / i) ) for i = 1 to n

2 long long math_block(long long n) {
3
4
5
6
...
r = n / (n / l); // 魔法公式：直接算出與 l 擁有相同 floor(n/i) 值的右邊
界
...
起始位置，未匹配為 −1。pi[i] 代表前綴 S[0 . . . i] 中最長的「相等真前後
綴」長度。
...
用法: 多字串匹配 (Trie + KMP)，必須是 0-based index。fail[i] 指向當

前狀態最長的、且在 Trie 中存在的後綴狀態。
...
設三角形頂點為 A(x1, y1), B(x2, y2), C(x3, y3)

點 A, B, C 的對邊長分別為 a, b, c
三角形面積為 ∆
重心為 (Gx, Gy)，內心為 (Ix, Iy)，
外心為 (Ox, Oy) 和垂心為 (Hx, Hy)
...
用 法: 使 用 倍 增 法， 利 用 二 進 位 的 特 性， 預 處 理 出 每 個 節 點 向 上 跳
...
for(int i = 1; i <= n; ++i) adj[i].clear(); // 若題目有多筆測資，必須初始
化
...
depth[1] = 0; // 選擇根節點 (通常題目為 1，或 0)，並初始化根節點的深度與
DFS 預處理
...
12 /*
13 ===== 原始 01 背包 =====
14 適用情況: 需要 backtrack
15 -> 從 dp[MAXN][MAXW] 開始檢查:
16 如果 dp[i][j] == dp[i-1][j] 表示沒有拿第 i 個，接著讓 i = i-1 繼續跑
17 否則我們有拿第 i 個，接著跳到 dp[i-1][j-w[i]]
18 重複直到 i=0 (沒東西了) 或 j=0 (重量用完了)
19 */
20 int dp[MAXN + 1][MAXW + 1];
21 memset(dp, 0, sizeof(dp)); // 初始化為 0
22 for (int i = 1; i <= MAXN; ++i) // 從考慮第一個物品到第 N 個物品
23 {
24
...
31 /*
32 ===== 滾動陣列優化 =====
33 概念: 在計算考慮第 i 個物品時，我們只會用到第 i-1 行的資料，可以看成陣列中的上一

筆舊資料

34 透過想法上的極致壓縮，覆寫無用資料空間，可將空間複雜度降至 O(W)
35 限制是 j 必須由大往小跑，如果由小至大會重複使用到陣列資料導致答案錯誤
36 */
37
38 int dp[MAXW + 1];
39 memset(dp, 0, sizeof(dp));
40 for(int i = 1; i <= MAXN; ++i) // 注意一樣是要跑 MAXN 次，我們只是把陣列大小減
...
45 /*
46 ===== 完全背包 (無限背包) =====
47 已知: 第 i 個物品重量為 wi，價值 vi；背包總容量 W，且每一個物品都可以重複 (無限
...
58 /*
59 ===== 分組背包 =====
60 第 i 組第 j 個物品重量為 wij，價值 vij；背包總容量 W；但每組只能取一個
61 */
62 int W;
63 int dp[maxw];
64 vector<vector<int>> w, v;
65
...
73 /*
74 ===== 多背包共用商品 =====
75 題目：N 個物品 (同一項商品可被多個人同時取用)，G 個人各自有容量為 Wi 的背包，求所
...
76
77 題解：每個人挑選互相獨立，不需重複計算。(查表)
78 1. 以所有商品跑一次標準一維 0/1 背包，每次讀入 Wi 與 Vi 時就 run 一次，建立 dp
...
79 2. 結束後跑一遍 dp[i] = max(dp[i], dp[i-1]) 確保單調性。
80 3. 讀入每個人的容量 W，總答案 ans += dp[W]。
81 */
82
83 // N 個物品，G 個人
84 // 假設最大可能的背包容量為 MAX_W
85 const int MAX_W = 100000;
86
87 int main() {
88
89
...
// 2. 確保單調遞增 (這步很關鍵，確保容量較大的人至少能拿到容量較小時的最大價
值)
...
123 /*
124 === 恰好裝滿問題 / 子集合和 ===
125 題目敘述：給定 N 樣物品，每件物品的重量分別為 Wi，問可不可以將物品分成等重的兩堆。
...
用法: 兩個序列的「公共子序列」，在序列 A 中的索引必定是嚴格遞增的，
所以可以將 B 中的每一個數字替換成他在 A 中的位置 (索引)，接著對新陣
列求 LIS，長度就會剛好等於 LCS
...
時間複雜度: O(KlogK)K 為元素配對數量
1 vector<int> lcs_to_lis(const vector<int>& A, const vector<int>& B) {
2
3
4
5
...
return lis; // LIS 長度與 LCS 一致，可直接 return lis.size(); 就是 LCS 的
長度
...
2 // 所以當作有 n - 1 個人，目標的 13 號移成 12
3 // 再者從 0 開始比較好算，所以目標 12 順移成 11
4
5 // O(n)
...
else if(mid < target) l = mid+1; // mid 在 target 左邊
else r = mid-1;

用法: 二分搜技巧
時間複雜度: O(logN )
1 Binary Search = 找答案, 但不一定是對陣列二分搜
2
3 // === 基本找值 ===
4 // 需要在 sorted array
5 while(l <= r){
...
9
10 }
11 // 可以直接用 lower_bound(st, ed, target) 找位置
12
13 // === 對答案二分搜 ===
14 /*
15 找 「最大值的最小值 Minimax」或 「最小值的最大值 Maximin」
16 要求答案具有單調性 即 X 合法，則 <= 或 >= X 者都合法
17 */
18
19 // === 實數二分搜 ===
20 /*
21 需要用 eps 來決定停止狀態 while(r - l > eps)
22 或者是固定迭代次數，例如 for 100 次後停下
23 */
24
25 // === 找第一個合法 (最小) ===
26 // 答案必在 [l, r] 間
...
用法: 用於 sorted array，找某個區間如兩數和 ==target
時間複雜度: O(N )
1 // === 對向雙指針 ===
2 int l = 0, r = n - 1;
3
...
10.4 Sliding Window 滑動窗口

用法: 同向雙指針的延伸，找最長/短合法區間，或固定長度下的最大區

間和

時間複雜度: O(N )
1 // 題意: 給定 target 與 array nums，求 nums 中 >= target 的 subarray，最小長度

是多少

2
3 // 題解: r 指針不斷向右跑，每次都加新數字進來；隨後判斷目前值是否超過 target，如
果超過就不斷地將最左側 (l 指針) 的數字剔除，最後回傳 len (我們維護的最小長度)
...
用法: 好用的東西可以參考
時間複雜度: N U LL
1 __builtin_popcount(x): 回傳整數 x 轉為二進位後 1 的數量 (若處理 long long

需使用

2 __builtin_popcountll(x)).
3 __builtin_clz(x): 回傳二進位前導 0 的個數 (Count Leading Zeros).
4 gcd(a, b), lcm(a, b): 求最大公因數 (C++17 引入, 需引入 <numeric>, 舊版編譯器
...
21 upper_bound(first, last, val): 尋找第一個 ">" x 的元素" 位置", 如果不存在,
...
25
26 unique(bg, ed): 將相鄰重複元素移至尾端, 回傳新陣列無重複部分尾端的 iterator (
...
77 s.erase(iterator first,iterator last) : 刪除 [first,last), 若沒有指定 last
...
用法: 有一個想法，也保證暴力解法全對時，可以使用對拍器檢查答案
時間複雜度: N U LL
1 // 使用當下時間作為亂數種子，確保每次執行生成的測資都不同
2 mt19937_64 rng(chrono::steady_clock::now().time_since_epoch().count());
3 /*
4 或 random_device rd; mt19937 rng(rd());
5 mt19937
...
11 }
12
13 // 1. 陣列與字串 (Array & String)
14
15 // 生成排列 (Permutation: 1 到 n 不重複)
16 void gen_permutation(int n) {
vector<int> p(n);
17
...
22 }
23
24 // 生成重複值很多的陣列 (distinct_count 控制相異數字的數量)
25 void gen_array_few_distinct(int n, long long L, long long R, int
...
33
34 // 生成已排序或完全反序的陣列
35 void gen_sorted_array(int n, long long L, long long R, bool reverse_sort =
...
43
44 // 生成隨機字串
45 void gen_string(int n, string charset = "abcdefghijklmnopqrstuvwxyz") {
46
47
48
49
...
52 // 2. 樹論結構生成器 (Trees)
53
54 // 輔助函式：打亂邊的順序與節點編號，避免特殊的編號順序讓錯誤的 Code 賽中
55 void print_tree(int n, vector<pair<int, int>>& edges, bool weighted = false,
...
68 }
69
70 // 鏈狀樹 (Chain / Bamboo)：一條線到底 (常卡遞迴深度)
71 void gen_chain(int n, bool weighted = false) {
72
73
74
75 }
76
77 // 星狀樹 (Star)：一個中心點連向所有其他點 (常卡度數度為 O(N) 的暴力解)
78 void gen_star(int n, bool weighted = false) {
79
80
81
82 }
83
84 // 完全二元樹 (Complete Binary Tree)：樹高最低的平衡樹
85 void gen_complete_binary_tree(int n, bool weighted = false) {
86
87
88
89 }
90
91 // 毛毛蟲樹 (Caterpillar)：一條主幹，其餘點全部直接連在主幹上
92 void gen_caterpillar(int n, bool weighted = false) {
93
94
95
96
...
104
105 // 有向無環圖 (DAG)
106 void gen_dag(int n, int m, bool weighted = false, long long max_w = 100) {
107
108
109
110
...
128 }
129
130 // 完全圖 (Complete Graph)：N 個點有 N*(N-1)/2 條邊 (常卡 O(E^2) 演算法)
131 void gen_complete_graph(int n, bool weighted = false, long long max_w = 100)
...
146 }
147
148 // 4. 生成隨機區間 [l, r] (Sweep Line, 區間 DP 題型)
149 void gen_intervals(int n, long long max_val) {
150
151
152
153
...
// [圖論 Edge Cases]
gen_dag(N, M, true); // 產生 10 點 15 邊的帶權 DAG
